...
// Импортируем useContext - чтобы получить доступ к Context
...
import { CONTACT_EMPLOYEES_FORM } from "./types";
import { EmployeeContext } from "../../EmployeeContext";

function EmployeesForm() {
  // 2. Подключаем контекст
  const employeeContext = useContext(EmployeeContext);
  // Проверка, чтобы TS не ругался (на случай, если контекст не найден)
  if (!employeeContext) return null;
  const { setUserData } = employeeContext;
